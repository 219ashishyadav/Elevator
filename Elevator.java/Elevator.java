...
public Elevator() {
this.currentFloor = 0;//assumption the lift is starting from Ground this.currentDirection = Direction.UP;//If at bottom , the lift will go up this.requestedPathsMap = new HashMap<>();
this.currentFloorDestinations = new Boolean[MAX_FLOOR + 1]; Arrays.fill(this.currentFloorDestinations, Boolean.FALSE);
}
...
private void processFloor(int floor) throws InterruptedException {
    ...
    requestedPathsMap.get(floor).forEach(destinationFloor -> currentFloorDestinations[destinationFloor] = true);//Marked true for next traversals requestedPathsMap.remove(floor);//removing the entry from map as we have marked all the destination
